    ...
    },
    {
      ...
      render: row => (
        <div class="flex gap-4">
          ...
          <NButton type="primary" size="small" onClick={() => analyzeResume(row.fileMd5)}>
            分析
          </NButton>
...
async function analyzeResume(fileMd5: string) {
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    <NCard title="简历管理" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
    ...
    <!-- 简历分析结果弹窗 -->
    <NModal v-model:show="analyzeVisible" preset="card" title="简历分析结果" style="width: 80%; max-width: 1000px">
      <div v-if="analysisResult" class="space-y-4">
        <div class="flex items-center gap-4">
          <span class="text-lg font-bold">综合评分:</span>
          <NTag :type="analysisResult.score >= 80 ? 'success' : analysisResult.score >= 60 ? 'warning' : 'error'" size="large">
            {{ analysisResult.score }}/100
          </NTag>
        </div>
        
        <div>
          <h3 class="text-md font-semibold mb-2">优势:</h3>
        ...
        <div>
          <h3 class="text-md font-semibold mb-2">改进空间:</h3>
          <ul class="list-disc pl-6 space-y-1">
        ...
        <div>
          <h3 class="text-md font-semibold mb-2">修改建议:</h3>
          <ul class="list-disc pl-6 space-y-1">
