    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (path.matches(".*/upload/chunk.*") ||
                    ...
                    path.matches(".*/admin/org-tags/tree.*") ||
                    path.matches(".*/users/org-tags.*") ||
                    path.matches(".*/users/upload-orgs.*")) {
                ...
                if (path.contains("/chunk")) {
                    ...
                    operation = "删除文档";
                } else if (path.contains("/admin/org-tags/tree")) {
                    operation = "获取组织标签树";
                } else if (path.contains("/users/org-tags")) {
                    operation = "获取用户组织标签";
                } else if (path.contains("/users/upload-orgs")) {
                    operation = "获取上传组织标签";
                }

                logger.info("处理{}请求: {}", operation, path);

                // 将用户ID、角色和组织标签设置为请求属性，供控制器方法使用
                ...
                if (token != null) {
                    ...
                    if (userId != null) {
                        ...
                        logger.debug("为{}请求设置属性: userId={}, role={}, orgTags={}", operation, userId, role, orgTags);
                    } else {
